...
import { getDiagrams } from 'apiSdk/diagrams';
...
function DashboardPage() {
  ...
  const diagramStat = useSWR<PaginatedInterface<DiagramInterface>>(
    () => `/diagrams`,
    () => getDiagrams({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Diagrams',
      result: diagramStat?.data?.totalCount,
      isLoading: diagramStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
