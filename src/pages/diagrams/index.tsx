...
import { getDiagrams, deleteDiagramById } from 'apiSdk/diagrams';
import { DiagramInterface } from 'interfaces/diagram';

type ColumnType = ColumnDef<DiagramInterface, unknown>;
...
export function DiagramListPage(props: DiagramListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DiagramInterface>>(
    () => `/diagrams?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DiagramInterface) => {
    if (hasAccess('diagram', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/diagrams/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/diagrams/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('diagram', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/diagrams/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('diagram', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="diagram" />
        </Flex>

        {hasAccess('diagram', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/diagrams/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
